...
function getCurrentList() {
  ...
  for (var i = 0; i < shop_list.list.length; i++) {
    var current_html = "<li key='".concat(i, "'>\n            <div class='name'>").concat(i + 1, ".").concat(shop_list.list[i].name, "</div>\n            <div class='price'>$").concat(shop_list.list[i].price, "</div>\n            <div class='delete' key='").concat(i, "'>X</div>\n        </li>");
    $(".list").append(current_html);
    total_price += shop_list.list[i].price;
  }

  var total_html = "<li class='total'>\n                <div class='name'>\u7E3D\u50F9:</div>\n               <div class='price'>$".concat(total_price, "</div>\n        </li>");
  $(".list").append(total_html); // 觸發刪除按鈕要包在getCurrentList(){}函式內，然後自己再呼叫一次
  // 如果在函式外面，只能觸發一次
  ...
  $(".delete").click(function () {
    var del_key = $(this).attr("key"); // 重新生成的key不算
